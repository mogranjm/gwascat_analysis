...
## Clean the corpus
clean_corpus <- title_corpus %>% 
	tm::tm_map(tm::removePunctuation, preserve_intra_word_dashes=TRUE) %>% 
	tm::tm_map(tm::removeNumbers) %>% 
	tm::tm_map(tm::content_transformer(tolower)) %>% 
	# tm::tm_map(tm::removeWords, c(tm::stopwords("en"))) %>%          # Removing "cancer" has a significant effect on the wordcloud
	tm::tm_map(tm::removeWords, c(tm::stopwords("en"), "cancer")) %>%
	tm::tm_map(tm::stripWhitespace)

for(i in 1:length(clean_corpus)){
	print(clean_corpus[[i]][1])
}

titles_dtm <- TermDocumentMatrix(clean_corpus)

titles_mat <- as.matrix(titles_dtm)

dim(titles_mat)

term_freq <- rowSums(titles_mat)
term_freq <- sort(term_freq, decreasing = T)

barplot(term_freq[1:10], col = 'purple', las=2)
...
titles %>% as_tibble() %>% filter(str_detect(value, " cell"))
```
