...
```{r}
gwas_cancer_data %>% filter(PUBMEDID == "28649644")
```

```{r}
bile_pubs <- filter(cancer_classes, bile_duct == TRUE)

breast_pubs[str_detect(breast_pubs$traits, chemotherapy),]


breast_pubs %>% 
	filter(str_detect(traits, "free")) #%>% 
	distinct(PUBMEDID)
```
...
```{r how many SNPs per publication}
SNPs <- breast_selected %>% 
	count(PUBMEDID) 
