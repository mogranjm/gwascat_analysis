...
gathered <- gwas_classified %>% 
    tidyr::gather(cancer, tf, biliary:cancer_unspecified) %>% 
    filter(tf==1) %>% 
    select(-c(tf, traits, start:strand))

```

```{r}
gathered %>% 
    distinct(cancer, DISEASE.TRAIT) %>% 
    arrange(cancer) %>% 
    write_csv(file.path(projroot, "reporting/gwas_cancer_classifications.csv"))
```

```{r}
gathered %>% 
    filter(str_detect(DISEASE.TRAIT, "[S|s]quamous")) %>% 
    distinct(PUBMEDID, DISEASE.TRAIT, cancer) %>% 
    arrange(PUBMEDID)
```
